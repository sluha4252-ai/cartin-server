...
# Головна сторінка чату (тепер точно без 404)
@app.get("/")
def get_chat_interface():
    ...
    <head>
        ...
        <style>
            ...
            .message { max-width: 75%; padding: 12px 16px; border-radius: 8px; font-size: 0.95em; line-height: 1.4; }
        ...
        <script>
            ...
            async function sendMessage() {
                ...
                try {
                    ...
                    appendMessage('Помилка з\'єднання.', 'bot');
                }
            }
        </script>
    </body>
...
async def chat_with_gemini(data: ChatMessage):
    ...
    payload = {
        ...
        "systemInstruction": {
            "parts": [{"text": "Ти — хлопець на ім'я Алекс (нік cartin). Твій стиль спілкування — максимально розслаблений (chill), дружній, трохи пофігістичний. Відповідай коротко, використовуй сленг. Спілкуйся тією мовою, якою до тебе звертаються."}]
        }
    }
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                return {"reply": f"Бот задумався... Код помилки: {response.status_code}"}
        except Exception:
            return {"reply": "Алекс кудись відійшов, спробуй ще раз."}
